...
public class MessageService {
    ...
    public void processMessage(MessageDto messageDto) {
        log.info("MessageService: обработка сообщения");
        ...
        if (webSocketService.isUserOnline(receiverId)) {
            log.info("Получатель userId={} онлайн, отправляем через WebSocket", receiverId);

            messageDto.setId(savedMsg.getId());
            webSocketService.sendMessageToUser(receiverId, messageDto);

            savedMsg.setStatus(MessageStatus.DELIVERED);
            messageRepository.save(savedMsg);

        } else {
            log.info("Получатель userId={} оффлайн, сообщение ждёт в БД", receiverId);
        }
    ...
    public void sendPendingMessage(Long userId) {
        log.info("Отправка накопившихся сообщений для userId={}", userId);
        List<MessageDto> pending = getUnreadMessage(userId);
        ...
        log.info("Найдено {} непрочитанных сообщений", pending.size());
        pending.forEach(msg -> {
                    webSocketService.sendMessageToUser(userId, msg);
                    Message message = messageRepository.findById(msg.getId()).orElse(null);
                    if (message != null) {
        ...
        log.info("Все непрочитанные сообщения отправлены");
